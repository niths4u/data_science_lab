...
- **SWARM Method** we can deploy it on swarm node by changing dockerspawner to swarmspawner and network settings in docker-compose.yaml and jupyterhubconfig.py. Kubernetes is over engineering for small setup like this. Once the changes are performed, any number of servers can be added. 
